...
final class PackageManagerActivity$filter$1 extends Lambda implements Function1<NeoPackageInfo, String> {
    public static final PackageManagerActivity$filter$1 INSTANCE = new PackageManagerActivity$filter$1();

    PackageManagerActivity$filter$1() {
        super(1);
    }

    public final String invoke(NeoPackageInfo neoPackageInfo) {
        Intrinsics.checkParameterIsNotNull(neoPackageInfo, "it");
        String packageName = neoPackageInfo.getPackageName();
        if (packageName == null) {
            Intrinsics.throwNpe();
        }
        return packageName;
