...
class NotificationBundleProcessor {
    ...
    private static void saveNotification(NotificationGenerationJob notificationGenerationJob, boolean z) {
        ...
        try {
            ...
            if (notificationGenerationJob.isNotificationToDisplay()) {
                ...
                instance.update(OneSignalDbContract.NotificationTable.TABLE_NAME, contentValues, "android_notification_id = " + notificationGenerationJob.getAndroidIdWithoutCreate(), (String[]) null);
                BadgeCountUpdater.update(instance, context);
            }
            ContentValues contentValues2 = new ContentValues();
            contentValues2.put(OneSignalDbContract.NotificationTable.COLUMN_NAME_NOTIFICATION_ID, customJSONObject.optString("i"));
    ...
    private static void processCollapseKey(NotificationGenerationJob notificationGenerationJob) {
        if (!notificationGenerationJob.restoring && notificationGenerationJob.jsonPayload.has("collapse_key") && !"do_not_collapse".equals(notificationGenerationJob.jsonPayload.optString("collapse_key"))) {
            ...
            Cursor query = OneSignalDbHelper.getInstance(notificationGenerationJob.context).query(OneSignalDbContract.NotificationTable.TABLE_NAME, new String[]{OneSignalDbContract.NotificationTable.COLUMN_NAME_ANDROID_NOTIFICATION_ID}, "collapse_id = ? AND dismissed = 0 AND opened = 0 ", new String[]{optString}, (String) null, (String) null, (String) null);
