...
public class GcmBroadcastReceiver extends WakefulBroadcastReceiver {
    ...
    private static boolean isGcmMessage(Intent intent) {
        ...
        if (stringExtra == null || GCM_TYPE.equals(stringExtra)) {
            return true;
        }
        return false;
    }
    ...
    static void startGCMService(Context context, Bundle bundle) {
        if (!NotificationBundleProcessor.hasRemoteResource(bundle)) {
            NotificationBundleProcessor.ProcessFromGCMIntentService(context, setCompatBundleForServer(bundle, BundleCompatFactory.getInstance()), (NotificationExtenderService.OverrideSettings) null);
            return;
        }
