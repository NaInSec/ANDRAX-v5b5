...
public class OSTrackerFactory {
    ...
    public void addSessionData(JSONObject jSONObject, List<OSInfluence> list) {
        for (OSInfluence next : list) {
            if (AnonymousClass1.$SwitchMap$com$onesignal$influence$model$OSInfluenceChannel[next.getInfluenceChannel().ordinal()] == 1) {
    ...
    public List<OSInfluence> getSessionInfluences() {
        ArrayList arrayList = new ArrayList();
        for (OSChannelTracker next : this.trackers.values()) {
            if (!(next instanceof OSInAppMessageTracker)) {
